...
CNY_RATE = 7.2       # USD→CNY（人民元）。中国語モードの表示用
...
def fetch_rate():
    ...
    try:
        ...
        with urllib.request.urlopen("https://open.er-api.com/v6/latest/USD", timeout=5) as r:
            ...
            JPY_RATE = float(rates.get("JPY", JPY_RATE))
            CNY_RATE = float(rates.get("CNY", CNY_RATE))
    except Exception:
        pass

def price(c):                      # 言語に合わせた通貨: en=$ / ja=¥(円) / zh=¥(人民元)
    if c is None: return "—"
    if _ui_lang == "en": return f"${c/100:.2f}"
    if _ui_lang == "zh": return f"¥{c / 100 * CNY_RATE:,.2f}"
